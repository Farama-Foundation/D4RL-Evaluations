...
class Actor(tf.keras.Model):
  ...
  def __init__(self, state_dim, action_dim):
    ...
    self.trunk = tf.keras.Sequential([
        tf.keras.layers.Conv2D(5, (5,5), activation='relu', input_shape=(48, 48, 3)),
...
class Critic(tf.keras.Model):
  ...
  def __init__(self, state_dim, action_dim):
    ...
    self.critic = tf.keras.Sequential([
        tf.keras.layers.Dense(
            256,
            input_shape=(hidden_dim + action_dim,),
            activation=tf.nn.relu,
            kernel_initializer='orthogonal'),
        tf.keras.layers.Dense(
            256, activation=tf.nn.relu, kernel_initializer='orthogonal'),
  ...
  def call(self, states, actions):
    ...
    """
    states = tf.reshape(states, [-1, 48, 48, 3])
...
class DoubleCritic(tf.keras.Model):
  ...
  def __init__(self, state_dim, action_dim):
    ...
    self.conv = tf.keras.Sequential([
        tf.keras.layers.Conv2D(5, (5,5), activation='relu', input_shape=(-1, 48, 48, 3)),
        tf.keras.layers.MaxPool2D(2),
        tf.keras.layers.Conv2D(5, (5,5), activation='relu'),
        tf.keras.layers.MaxPool2D(2),
        tf.keras.layers.Flatten(),
